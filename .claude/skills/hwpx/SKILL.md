...
> [!IMPORTANT] 이 머신 전용 (로컬 추가)
> - 이 스킬 문서의 python 호출과 스크립트 셔뱅은 전부
>   `/Users/clara/.claude/skills-venv/bin/python` (3.12) 으로 이미 바꿔뒀다. 그대로 쓰면 된다.
>   시스템 `python3` 는 3.9.6 이라 `match` 문에서 죽는다. PATH 는 바꾸지 않았다 —
>   3.9 에 vrew/whisper 용 mlx_whisper·torch 스택이 있어서 건드리면 그쪽이 깨진다.
> - 패키지 추가: `uv pip install --python /Users/clara/.claude/skills-venv/bin/python <pkg>`
...
/Users/clara/.claude/skills-venv/bin/python scripts/convert_hwp.py <ref.hwp> -o <ref.hwpx>

# [1] 레퍼런스에서 편집 규범 추출 (스타일 공급용 base.hwpx 도 함께 보관)
/Users/clara/.claude/skills-venv/bin/python scripts/doc_spec.py analyze <ref.hwpx> -o spec/

# [2] 원고를 그 규범으로 조판
/Users/clara/.claude/skills-venv/bin/python scripts/doc_spec.py render spec/ <content.md> -o out.hwpx \
...
```bash
/Users/clara/.claude/skills-venv/bin/python scripts/doc_spec.py lint <out.hwpx>
```
...
/Users/clara/.claude/skills-venv/bin/python scripts/map_preflight.py dump <base.hwpx> [--grep 검색어]

# [2] replace 전 사전검증 — 못 맞출 키를 원인과 교정안까지 짚어준다
/Users/clara/.claude/skills-venv/bin/python scripts/map_preflight.py check <base.hwpx> --map map.json [--fix fixed.json]
...
>
> ```bash
> /Users/clara/.claude/skills-venv/bin/python scripts/bodojaryo.py --sample --output 보도자료.hwpx        # 샘플
> /Users/clara/.claude/skills-venv/bin/python scripts/bodojaryo.py --input bodo.json --output 보도자료.hwpx  # JSON 입력
> /Users/clara/.claude/skills-venv/bin/python scripts/gonmun_lint.py --hwpx 보도자료.hwpx --format text       # 본문 작성법 검수
> ```
...
```bash
/Users/clara/.claude/skills-venv/bin/python scripts/yoyak.py 보고.md -o 보고.hwpx          # 마크다운 → HWPX
/Users/clara/.claude/skills-venv/bin/python scripts/yoyak.py --emit-sample > 보고.md        # 문법 예시 뽑기
```
...
```bash
/Users/clara/.claude/skills-venv/bin/python scripts/geomto.py 계획.md -o 계획.hwpx
/Users/clara/.claude/skills-venv/bin/python scripts/geomto.py --emit-sample > 계획.md
```
...
```bash
/Users/clara/.claude/skills-venv/bin/python scripts/munche_lint.py 원고.md        # 위반(error) 있으면 exit 2 — 고친 뒤 조판
```
...
>
> ```bash
> /Users/clara/.claude/skills-venv/bin/python scripts/gonmun.py --sample --output 기안문.hwpx        # 샘플
> /Users/clara/.claude/skills-venv/bin/python scripts/gonmun.py --input gonmun.json --output out.hwpx  # JSON 입력
> ```
...
```bash
/Users/clara/.claude/skills-venv/bin/python scripts/gonmun_lint.py --hwpx 문서.hwpx --format text   # .hwpx 검수
/Users/clara/.claude/skills-venv/bin/python scripts/gonmun_lint.py --file 본문.txt                  # 텍스트 파일
echo "2025.1.6 오후 3시 회의" | /Users/clara/.claude/skills-venv/bin/python scripts/gonmun_lint.py   # 표준입력(JSON 출력)
```
