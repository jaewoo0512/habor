...
/Users/clara/.claude/skills-venv/bin/python scripts/fill_hwpx.py check output.hwpx     # exit 0=정상, 2=secPr 문제
```
...
| 이미 변환된 .hwpx에 테두리가 남음 | `/Users/clara/.claude/skills-venv/bin/python scripts/fill_hwpx.py fix-borders 파일.hwpx` |
| 표 셀 테두리까지 사라질까 걱정 | tc(표 셀)의 borderFillIDRef는 section에 있어 보존됨 |

```bash
# 이미 변환된 파일 보정 (표 테두리는 유지, 글자 테두리만 제거)
/Users/clara/.claude/skills-venv/bin/python scripts/fill_hwpx.py fix-borders 변환본.hwpx              # 덮어쓰기
/Users/clara/.claude/skills-venv/bin/python scripts/fill_hwpx.py fix-borders 변환본.hwpx out.hwpx     # 새 파일로

# 배포 전 점검 — 글자 테두리도 함께 검사 (strict면 차단)
/Users/clara/.claude/skills-venv/bin/python scripts/fill_hwpx.py check 파일.hwpx --strict
```
...
| Cause | Fix |
|------|-----|
| Stale `hp:linesegarray` layout caches remain after editing text outside Hancom | Run `/Users/clara/.claude/skills-venv/bin/python scripts/finalize_hwpx.py output.hwpx --strip-linesegarray` before validation |
| XML validation passed but Hancom still refuses the file | Run `/Users/clara/.claude/skills-venv/bin/python scripts/validate.py output.hwpx --hancom` on Windows with Hancom Office installed |
...
```bash
/Users/clara/.claude/skills-venv/bin/python scripts/finalize_hwpx.py output.hwpx --layout
```
